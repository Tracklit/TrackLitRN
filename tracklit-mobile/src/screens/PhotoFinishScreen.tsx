  ...
  const handleAIAnalysis = useCallback(async (analysis: FrameAnalysis) => {
    ...
    try {
      ...
      if (!fetchResponse.ok) {
        ...
        throw new Error(
          errorData?.error || errorData?.message || `Server error (${fetchResponse.status})`
        );
      }
      ...
      const isFallback = fallbackPhrases.some(p => aiText.toLowerCase().includes(p));

      if (isFallback || !aiText) {
        setAiError('The AI analysis service is temporarily unavailable. Please try again later.');
      } else {
        setAiResult(aiText);
      }
    } catch (err: any) {
      if (err?.name === 'AbortError') {
        setAiError('Analysis request timed out. The AI service may be busy — please try again.');
      } else {
