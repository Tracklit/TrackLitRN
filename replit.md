...
- **Navigation:** Bottom navigation features 5 tabs (Home, Training, Feed, Tools, Sprinthia), with Profile accessible via a drawer. The Training tab is a unified screen with Practice and Programs sub-tabs. Sprinthia is a proper Tab screen (not a RootStack screen) so it behaves identically to the other main tabs.
...
- **Photo Finish:** A core feature allowing video upload, frame scrubbing, digital clock overlays, FPS selection, and advanced analysis including MediaPipe pose skeleton overlay, joint angle computation, body symmetry, center of mass, and motion tracking. It includes a frame comparison tool and AI analysis via the Sprinthia API. The scrubber bar has a visible background track, plus dedicated Play/Pause and frame-step (SkipBack/SkipForward) buttons. Pose detection shows a loading indicator when processing and a "No athlete detected" message when detection fails. The Analysis modal is a half-screen bottom sheet (65% height, transparent backdrop, tap outside to dismiss). The AdvancedAnalysis component has a new "Performance" tab computing ground contact time, stride length, cadence (spm), and horizontal velocity from landmark history.
